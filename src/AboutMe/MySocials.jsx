      ...
      <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-4">
        {[
         ...
         <a key={index} href={icon.link} target="_blank" rel="noopener noreferrer">
          <img
            src={icon.src}  alt={icon.alt} className="w-18 h-16 md:w-14 md:h-14 object-contain flex-shrink-0 transition-all duration-300 hover:scale-150"/>
        </a> ))}
       </div>

      <h1 className="hidden sm:block font-serif italic text-center text-2xl mt-5">Accounts</h1>
      <button
        className="absolute top-2 right-2 text-lg p-1 rounded-sm cursor-pointer"
        onClick={remMenu}>
        <MdOutlineCancel />
      </button>
    </section>
